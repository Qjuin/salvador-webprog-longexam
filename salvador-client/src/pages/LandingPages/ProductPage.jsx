...
function ProductPage() {
  ...
  return (
    <div className="flex w-full flex-col gap-6">
      ...
      <section className="border-y-2 border-blue-900 bg-blue-50 px-4 py-7 sm:px-6 sm:py-9 lg:px-8">
        <div className="mx-auto max-w-3xl">
          <div className="mb-8 overflow-hidden rounded-[1.25rem] border-2 border-slate-200 bg-slate-100">
            {product.image ? (
              <img
                src={product.image}
                alt={product.title}
              ...
              <div className="flex aspect-4/3 items-center justify-center">
                <div className="flex h-24 w-24 items-center justify-center border-2 border-slate-200 bg-white text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500">
                  Item
                </div>
              </div>
            )}
          ...
          <div className="prose prose-sm max-w-none space-y-4 text-slate-700">
            {product.content.map((paragraph, index) => (
              <p key={index} className="whitespace-pre-wrap text-base leading-7 text-slate-700">
                {paragraph}
              </p>
            ))}
          </div>
